...
def test_build_capabilities_blocks_at_max_per_user(policy_limits):
    caps = build_capabilities("administrator", total_zones=3)
    assert caps.can_create_zone is False
    assert caps.remaining_total == 0
    assert caps.remaining_for_role == 0
    assert caps.reason == "Maximum of 3 zones per user reached."
...
@pytest.mark.asyncio
async def test_user_cannot_create_more_than_three_zones(zone_test_db, policy_limits):
...
@pytest.mark.asyncio
async def test_each_user_has_independent_three_zone_quota(zone_test_db, policy_limits):
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        assert payload["reason"] == "Maximum of 3 zones per user reached."

        user_caps = await client.get("/zones/capabilities", headers=user_headers)
        assert user_caps.status_code == 200
        assert user_caps.json()["can_create_zone"] is True
        assert user_caps.json()["remaining_total"] == 2
...
async def test_concurrent_create_at_boundary_allows_single_success(zone_test_db, policy_limits):
    ...
    try:
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            await client.post("/zones/", headers=headers, json=_zone_payload("Existing"))
