...
class OwnerContractResponse(BaseModel):
    ...
    id: str
    network_id: str = Field(serialization_alias="networkId")
    email: EmailStr
    zone_id: str
    first_name: str
    last_name: str
...
async def get_me(owner: Owner = Depends(require_auth), db: Session = Depends(get_db)):
    ...
    data = OwnerContractResponse(
        id=str(owner.id),
        network_id=owner.network_id or "",
        email=owner.email,
    ...
    return success_response(data.model_dump(by_alias=True))
...
def _seed_empty_shared_fields(
    ...
    - Network ID     -> ``owners.network_id`` (per-user identifier)
    ...
    if not sn.network_id:
        sn.network_id = (owner.network_id or "").strip()
...
@router.get(
    ...
    description=(
        ...
        "smart-home device, network id and account api key; quick messages come from the "
...
@router.put(
    ...
    description=(
        ...
        "to `owners.network_id` only for Private (system administrator) accounts."
    ),
)
async def put_my_settings(
    ...
    if requested_network_id and requested_network_id != (owner.network_id or "").strip():
        assert_owner_may_edit_network_id(owner)
        owner.network_id = requested_network_id
