...
from app.routers import access, app_links, devices, guest, message_feature, messages, owners, utils, zones
...
OPENAPI_TAGS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "**`POST /api/access/guest-passes`** (event_id + expiry). A new unique **`event_id`** is "
            "**ACCEPTED** immediately (no admin review). Admins may revoke with "
...
app = FastAPI(
    ...
    description=(
        ...
        "`POST /api/access/guest-passes` (event_id + expiry). A new unique event_id is **ACCEPTED** immediately. "
        "Admins revoke via `POST /api/access/guest-passes/{id}/revoke`. **`revoke`** on a consumed pass invalidates that guest's **`/api/guest/*`** Bearer (**`401`** **`GUEST_ACCESS_INVALIDATED`**). "
