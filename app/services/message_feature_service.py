...
def resolve_geo_propagation_recipient_owner_ids(
    ...
    *,
    ...
    1. GPS must fall inside a zone polygon (permission gate).
    2. Resolve the zone id label(s) for that geometry.
    3. Deliver / search all active owners whose profile ``zone_id`` matches.
    Recipient GPS is not required.
    """
    ...
    unique_zone_ids = list(dict.fromkeys(str(z).strip() for z in zone_ids if str(z).strip()))
    if not zone_record_ids or not unique_zone_ids:
        return [], [], [], {
    ...
    recipient_ids = set(_owner_ids_for_zone_id_labels(db, unique_zone_ids))
    ...
    meta = {
        "strategy": "matching_zone_id_owners",
        "sender_zone_ids": unique_zone_ids,
        "sender_zone_record_ids": zone_record_ids,
        "recipient_owner_ids": sorted_recipients,
    }
    return unique_zone_ids, zone_record_ids, sorted_recipients, meta
...
def _zone_based_recipients(
    ...
    Uses the same zone-id pool as PRIVATE search: all active owners whose profile
    ``zone_id`` matches the zone label(s) at the sender's GPS. Recipient GPS is
    not required.

    For PRIVATE scope the sender must be inside a zone; exactly one selected
    owner from that pool receives the message.
    """
